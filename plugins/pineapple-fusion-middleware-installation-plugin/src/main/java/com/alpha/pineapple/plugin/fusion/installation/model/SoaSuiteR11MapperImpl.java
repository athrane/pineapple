 ...
 * of the SOA Suite 11.x installer from the content of the model. 
 */
public class SoaSuiteR11MapperImpl implements ModelMapper {
	...
	public String getInstaller(FusionMiddlewareInstallation model) {
		return model.getSoaSuite().getRelease11().getInstaller();		
	}
	
	public String getUninstaller(FusionMiddlewareInstallation model) {
		return model.getSoaSuite().getRelease11().getUninstaller();
	}

	public File getTargetDirectory(FusionMiddlewareInstallation model) {
		return new File(model.getSoaSuite().getRelease11().getTargetDirectory());		
	}

	public File getLocalJvm(FusionMiddlewareInstallation model) {
		return new File(model.getSoaSuite().getRelease11().getLocalJvm());
	}	
