...
public interface ResultRepository extends ExecutionResultFactory {
     ...
     * 
     * Creates, stores and returns a {@link ExecutionInfo} object which contains
     * the passed arguments and is initialized with a new {@link ExecutonResult}
     * whose state is running.
     * 
     * The execution info object is initialized with the modules directory that
     * the repository is previously configured with. If the modules directory
     * isn't configured then an exception is thrown.
     * 
     * @param moduleInfo
    ...
    /**
     * Start composite execution of operation on and module. The composite
     * execution is attached to existing execution.
     * 
     * Creates, stores and returns a {@link ExecutionInfo} object which contains
     * the passed arguments and is initialized with a new {@link ExecutonResult}
     * whose state is running.
     ...
     * execution of operation about how they proceeds.
     * 
     * @param listener
    ...
    /**
     * Get result notification sequence where execution results are sorted in
     * the order of which they are created.
     * 
     ...
     * 
     *            Querying with indexes 0..0 returns the empty sequence.
     *            Querying with indexes 0..1 returns a single result.
     * 
     ...
     * 
     * @return the current (maximum) index used store the last result for
     *         execution.
     * 
     * @throws ExecutionInfoNotFoundException
     *             if execution info is unknown.
