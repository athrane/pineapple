...
Please notice that the administration will listen ONLY on the HTTPs listen port 
(whose configuration is focus of this example).  
...
b) Since the server is configured to use DEMO trust in step 2) the WLST must
connect to the running server in step 3) using DEMO trust. This is done by 
setting the attribute demo-trust-enabled="true" when WLSt is invoked in step 3.
