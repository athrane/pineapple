...
public class CreateContainerCommandSystemTest {
    ...
    /**
     * Test that command can create container image with minimal configuration (image only)
     * and return a single successful root result.
     */
	...
	InspectedContainer inspectedContainer = dockerHelper.inspectContainer(session, containerInfo);
	...
	List<String> inspectedEntryPoint = inspectedContainerConfig.getEntrypoint();
    ...
    /**
     * Test that command can create container with zero commands.
     * The result command is inherited from the CMD directive in the CentOS Dockerfile.
     */
    @SuppressWarnings("unchecked")
    @Test
	...
	InspectedContainerConfiguration inspectedContainerConfig = inspectedContainer.getConfig();
	assertNotNull(inspectedContainerConfig);
	List<String> inspectedCmds=inspectedContainerConfig.getCmd();
	assertNotNull(inspectedCmds);
	assertEquals(1, inspectedCmds.size());
    }
    
    /**
     * Test that command can create Registry container with zero commands.
     * The result command is inherited from the CMD directive in the registry Dockerfile.
     */
    @SuppressWarnings("unchecked")
    @Test
	...
	ContainerInfo containerInfo = dockerInfoBuilder.buildContainerInfo(randomId, imageInfo);
	ContainerConfiguration config = containerInfo.getContainerConfiguration();
	if(config.getHostConfig() == null) config.setHostConfig(dockerModelObjectFactory.createContainerConfigurationHostConfig());
