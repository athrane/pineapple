...
public class DeployConfigurationIntegrationTest {
	...
	/**
	 * Write mock installer.
	 *  
	 * @return File which defines path to mock installer.
	 * 
	 * @throws IOException If mock installer creation fails. 
	 */
	...
	public void testCanExecuteWithR28Model() throws Exception {
		...
		// complete mock runtime coreRuntimeDirectoryProvider setup
		EasyMock.expect(coreRuntimeDirectoryProvider.getTempDirectory()).andReturn(testDirectory).times(3);
		EasyMock.expect(coreRuntimeDirectoryProvider.startsWithModulePathPrefix(installerFile.getAbsolutePath())).andReturn(false);
		EasyMock.replay(coreRuntimeDirectoryProvider);
