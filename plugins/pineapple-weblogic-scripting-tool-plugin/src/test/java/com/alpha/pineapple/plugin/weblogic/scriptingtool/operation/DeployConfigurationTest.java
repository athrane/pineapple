...
/**
 * Unit test for the <code>DeployConfiguration</code> class.  
 */
...
public class DeployConfigurationTest {
	...
	void completeMockSessionSetup2(String[] arguments) {
		EasyMock.expect(session.getResource()).andReturn(resource);
		session.execute(randomWlstInvoker.getAbsolutePath(), arguments, DEFAULT_PROCESS_TIMEOUT, "dc.execute", executionResult);				
		EasyMock.replay( session );
	}		
