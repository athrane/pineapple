 ...
 * 1) The two maps contains the same number of keys.
 * 2) The two maps contains the same keys names. 
 * 3) A key maps to the same value in both maps.
 * </p>  
 */
public class IsCookieMapsPairContainingEqualValues extends TypeSafeMatcher<CookieMapPair> {
	...
	protected boolean matchesSafely(CookieMapPair pair) {		
		...
		if (!InfrastructureMatchers.cookieMapsPairOfEqualSize().matches(pair)) return false; 

		// succeeds if maps are empty
		if (Matchers.empty().matches(pair.getFirstMap().keySet())) return true;
		
		// fails if the maps contains different key sets
		if (!InfrastructureMatchers.cookieMapsPairContainingEqualKeys().matches(pair)) return false; 
	...
	/**
	 * Returns true if the same key maps to the same value in the two maps.
	 * Return false if a least one key map to different values in the two maps.
	 * 
	 * @param pair
	 *            Pair of cookie maps.
	 * 
	 * @return true if the same key maps to the same value in the two maps.
	 *         Return false if a least one key map to different values in the
	 *         two maps.
	 */
    ...
    /**
     * Computes the difference between two cookie maps, 
     * by removing all the cookies from map #1 which is
     * found is map #2. A cookie is removed from map #1
     * if a cookie with identical name is found in map #2,
     * and the values of the two cookies are equal.
     * 
     * @param map1 Cookie map #1.
     * @param map2 Cookie map #2.
     * 
     * @return Array of cookies from Map #1 whose values are
     * different from the corresponding cookie in map #2.  
     */
