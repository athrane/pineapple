...
public class JettyStarter {
	...
	/**
	 * JettyStarter no-arg constructor which start an embedded Jetty web server instance.
	 *
	 *<p>The class must be invoked with these system properties defined:
	 *
	 * <ul>
	 * <li><code>pineapple.jettystarter.home</code> defines home directory for Jetty. The type is
	 * <code>java.lang.String</code>.</li>
	 * <li><code>pineapple.jettystarter.host</code> defines host name which the Jetty server should listen on. The type is
	 * <code>java.lang.String</code>.</li>
	 * <li><code>pineapple.jettystarter.port</code> defines port number on which the Jetty server should listen on. The type is
	 * <code>integer</code>.</li>
	 * </ul>  
	 * </p> 
	 *  
	 *<p>The class can be invoked with these system properties defined:
	 *
	 * <ul>
	 * <li><code>pineapple.jettystarter.stdoutlogging</code> defines whether the Jetty starter should log to standard out.
	 * if the property is defined and the value is "true" then the logging to standard out is enabled. 
	 * <code>java.lang.String</code>.</li>
	 * </ul>  
	 * </p> 
	 *  
	 * @param args command line arguments. Not used by the class.
	 * 
	 * @throws Exception If starting the Jetty server fails.  
	 */
	 ...
	 * 
	 * @param serverHost Server host name.
	 * @param serverPort Server port.
	 * @param stdLogging if string contains the string "true" 
	 * then message is logged to standard out.
	 * 
	 * @throws IOException
	 */
	void launchBrowser(String serverHost, String serverPort, String stdLogging)
	{
		...
		if(!java.awt.Desktop.isDesktopSupported()) {
			String message = new StringBuilder()
				.append("No desktop support - please open the application at: ")
		...
		try {
			...
			String message = new StringBuilder()
				.append("Failed to open application url [")
				.append(pageUrl.toString())
				.append("] due to exception:")
				.append(StackTraceHelper.getStrackTrace(e))
				.toString();
			logger.info(message);			
			logToStdOut(message,stdLogging);						
		}
	...
	void logToStdOut(String message, String stdLogging) {
		if(stdLogging == null) return;
		if(!stdLogging.equalsIgnoreCase("true")) return;
