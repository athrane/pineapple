...
/**
 * Implementation of the <code>Mapper</code> interface which map model 
 * content to command context.
 */
public class MapperImpl implements Mapper {

	public void mapModel(AgentCommand command, Context context, AgentSession session) {		
		if(command instanceof DistributeModule) mapDistributeModule((DistributeModule)command, context, session);		
		if(command instanceof ExecuteOperation) mapExecuteOperation((ExecuteOperation )command, context, session);
		if(command instanceof CreateEnvironment) mapCreateEnvironment((CreateEnvironment)command, context, session);							
	}
