 ...
 *   
 *    T3JmxServiceUrlFactoryTest.java created by edbuata on Sep 21, 2012 
...
/**
 * Integration test of the {@linkplain T3JmxServiceUrlFactoryImpl} class. 
 */
@RunWith( SpringJUnit4ClassRunner.class )
@ContextConfiguration( locations = { "/com.alpha.pineapple.plugin.weblogic.jmx-config.xml" } )
public class T3JmxServiceUrlFactoryIntegrationTest {

	/**
	 * Subject under test. 
	 */
	@Resource(name="t3JmxServiceUrlFactory")
	...
	public void testConnectionPropertiesContainsExpectedNumberProperties() {
		...
		assertEquals(WEBLOGIC_PROTOCOL_PROVIDER_PACKAGES , props.get(JMXConnectorFactory.PROTOCOL_PROVIDER_PACKAGES));		
	}

	/**
	 * Test service URL is created as expected for T3.
	 * 
	 * @throws Exception If test fails.
	 */
	@Test
	public void testCreateEditMBeanServerUrlForT3() throws Exception {
		...
		JMXServiceURL url = factory.createEditMBeanServerUrl(JmxProtool.T3, randomHost, randomPort);				
		assertEquals(url.toString(), expected.toString());
	}

	/**
	 * Test service URL is created as expected for T3S.
	 * 
	 * @throws Exception If test fails.
	 */
	@Test
	public void testCreateEditMBeanServerUrlForT3S() throws Exception {
		StringBuilder expected = new StringBuilder()
			.append("service:jmx:")
			.append("t3s")
			.append("://")
		...
		JMXServiceURL url = factory.createEditMBeanServerUrl(JmxProtool.T3S, randomHost, randomPort);
		assertEquals(url.toString(), expected.toString());
	}
