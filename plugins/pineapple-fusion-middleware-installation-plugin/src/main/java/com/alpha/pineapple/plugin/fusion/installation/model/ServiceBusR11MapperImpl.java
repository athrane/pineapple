 ...
 * of the Oracle Servie Bus 11.x installer from the content of the model. 
 */
public class ServiceBusR11MapperImpl implements ModelMapper {
	...
	public String getInstaller(FusionMiddlewareInstallation model) {
		return model.getSoaOsb().getRelease11().getInstaller();		
	}
	
	public String getUninstaller(FusionMiddlewareInstallation model) {
		return model.getSoaOsb().getRelease11().getUninstaller();
	}

	public File getTargetDirectory(FusionMiddlewareInstallation model) {
		return new File(model.getSoaOsb().getRelease11().getTargetDirectory());		
	}

	public File getLocalJvm(FusionMiddlewareInstallation model) {
		return new File(model.getSoaOsb().getRelease11().getLocalJvm());
	}	
