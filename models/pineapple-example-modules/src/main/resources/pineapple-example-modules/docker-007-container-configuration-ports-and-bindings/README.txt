...
Description
-------------
This example illustrates how the Docker plugin supports configuration of Docker containers.
A Docker container is created from a Dockerfile with HTTPD from CentOS base image.
The exposed port of HTTPD is configured in the module model.
...
The dockerfile will direct the installation of HTTPD, installation of a simple "Hello HTTPD" page 
and exposure of port 80 from the container.

The model contains container configuration of the exposed ports and port bindings.
The container exposes port 80 (also done in the Dockerfile with the EXPOSE 80 command).
The container defines a TCP port binding from container portr 80 to host port 8080.
