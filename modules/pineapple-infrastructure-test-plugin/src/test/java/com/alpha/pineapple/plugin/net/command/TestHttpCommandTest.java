...
public class TestHttpCommandTest
{
    ...
    public void setUp() throws Exception
    {
        ...
        // inject command into command
        ReflectionTestUtils.setField( command, "testResponsePropertiesCommand", testResponsePropertiesCommand, Command.class );
    ...
    public void testResultKeysIsDefinedInContextAfterExecution()
    {
        ...
        try
        {
    		...
    		// create capture to snap response properties when InvokeHttpGetMethodCommand is invoked  
    		...
    		EasyMock.expect( childContext.put(InvokeHttpGetMethodCommand.REQUESTS_KEY, requests )).andReturn(PUT_RETURN_VALUE);
    		EasyMock.expect( childContext.put(InvokeHttpGetMethodCommand.RESET_KEY, true )).andReturn(PUT_RETURN_VALUE);
    		EasyMock.expect( childContext.put(InvokeHttpGetMethodCommand.HTTPCONFIGURATION_KEY, httpConfig )).andReturn(PUT_RETURN_VALUE);
    		EasyMock.expect( childContext.put(
    				(String) EasyMock.eq(InvokeHttpGetMethodCommand.RESPONSEPROPERTIES_SET_KEY),
    				EasyMock.capture(responsePropsCapture))).andReturn(PUT_RETURN_VALUE);
    		...
    		// create capture to catch response properties when TestResponsePropertiesCommand is invoked  
    		...
    		EasyMock.expect( childContext2.put(
    				(String) EasyMock.eq(TestResponsePropertiesCommand.RESPONSEPROPERTIES_SET_KEY),
    				EasyMock.capture(responsePropsCapture2))).andReturn(PUT_RETURN_VALUE);
    		EasyMock.expect( childContext2.put(
    				(String) EasyMock.eq(TestResponsePropertiesCommand.INVOCATIONRESULTS_SET_KEY),
    				(HttpInvocationsSet) EasyMock.eq(httpInvocationsSet) )).andReturn(PUT_RETURN_VALUE);
    		...
    		EasyMock.expect( runner.run((Command) EasyMock.eq(httpGetCommand),
    									(String) EasyMock.isA( String.class ),
    		...
    		EasyMock.expect( runner.run((Command) EasyMock.eq(testResponsePropertiesCommand),
					(String) EasyMock.isA( String.class ),
					(Context) EasyMock.eq(childContext2)));    		
