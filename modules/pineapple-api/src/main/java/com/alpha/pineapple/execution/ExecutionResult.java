...
public interface ExecutionResult {
	 ...
	 * 
	 * @return Added result object with supplied description and in running state.
	 */
	 ...
	 * cancellation). Will update the cancellation state on the continuation policy.
	 * 
	 * The state of the result (and children) will not be updated untilexplict set
	 * or a child is created.
	 */
	 ...
	 * 
	 * @return the time that the operation took to execute. If the execution isn't
	 *         completed yet, then the current running time is returned.
	 */
	public long getTime();
	...
	/**
	 * Add detailed message about the outcome of the execution in a human readable
	 * form.
	 * 
	 * If a message with the key already exists then the new message is appended to
	 * the existing message separated by a line separator.
	 * 
	 * @param key
	...
	/**
	 * Replace message about the outcome of the execution in a human readable form.
	 * 
	...
	/**
	 * Returns all children which is registered with the requested execution state.
	 * If the execution result has no children registered with the requested state,
	 * then an empty array is returned.
	 * 
	 * @param state
	 *            Execution state which is requested.
	 * 
	 * @return Returns all children which is registered with the requested execution
	 *         state.
	 */
	public ExecutionResult[] getChildrenWithState(ExecutionState state);

	/**
	 * Return first child. If the execution result has no children registered then
	 * null is returned.
	 * 
	 * @return first child.
	 */
	...
	/**
	 * Set the state as error and add a message which is looked up from the message
	 * provider object. The looked up message is stored under the id 'Message' and
	 * 'Error Message'. A stack trace is extracted from the exception and stored
	 * under the id 'StackTrace'.
	 * 
	 * @param messageProvider
	 *            The message provider used to look up the message.
	 * 
	 ...
	 * description The message is stored under the id 'Message' and 'Error Message'.
	 * A stack trace is extracted from the exception and stored under the id
	 * 'StackTrace'.
	 * 
	 * @param exception
	 *            Exception from which the description and stack trace is extracted.
	 */
	public void completeAsError(Exception e);

	/**
	 * Set the state as error and add a message which is looked up from the message
	 * provider object. The looked up message is stored under the id 'Message'. A
	 * stack trace is extracted from the exception and stored under the id
	 * 'StackTrace'.
	 * 
	 * @param messageProvider
	 *            The message provider used to look up the message.
	 ...
	 * argument will contain the number of failed children and the second argument
	 * will contain the number children with error. If the failed message is defined
	 * as <code>XYZ failed, because [{0}] child executions failed and 
	 * [{1}] child executions terminated with an error.</code> then the values in
	 * argument 0 and 1 will be resolved by the method.
	 * 
	 * @param messageProvider
	 *            The message provider used to look up the message.
	 * 
	 * @param successfulKey
	 *            The key to lookup up in the message provider, such as
	 *            'calculator.noRateSet'. The key is only used to lookup a message
	 *            if the state is computed to successful
	 * 
	 * @param failedKey
	 *            The key to lookup up in the message source, such as
	 *            'calculator.noRateSet'. The key is used to lookup a message if the
	 *            state is computed to failure or error.
	 * 
	 * @param successfulArgs
	 *            Array of arguments that will be filled in for params within the
	 *            successful message (params look like "{0}", "{1,date}", "{2,time}"
	 *            within a message), or null if none.
	 * 
	 * @param failedArgs
	 *            Array of arguments that will be filled in for params within the
	 *            failed message (params look like "{0}", "{1,date}", "{2,time}"
	 *            within a message), or null if none.
	 */
	...
	/**
	 * Set the state as interrupted and add a message which is looked up from the
	 * message provider object. The looked up message is stored under the id
	 * 'Message'. A message which details the results of the children is stored
	 * under the id 'Composite execution result'.
	 * 
