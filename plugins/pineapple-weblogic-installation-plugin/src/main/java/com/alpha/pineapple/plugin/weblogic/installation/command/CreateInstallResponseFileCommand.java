...
import static com.alpha.pineapple.plugin.weblogic.installation.WeblogicInstallationConstants.RESPONSE_INSTALL_FILE;
...
import com.alpha.pineapple.plugin.weblogic.installation.response.UniversalInstallerResponseFileBuilder;
 ...
 * creates a response file for installation of WebLogic (12.1.2+) using the Oracle universal installer.
 * 
...
public class CreateInstallResponseFileCommand implements Command {
     ...
     */
    UniversalInstallerResponseFileBuilder responseFileBuilder;
     ...
     */
    public void setResponseFileBuilder(UniversalInstallerResponseFileBuilder responseFileBuilder) {
    ...
    {    	    	
		...
		if(responseFile.exists()) {
			...
			if(!succeded) {
		    	...
		    	executionResult.completeAsFailure(messageProvider, "cirfc.delete_oldresponsefile_failed", args);
		    	return;				
			}			
		}
		
		// ORACLE_HOME is interpreted as the parent directory of target directory.		
		File oracleHomeDirectory = targetDirectory.getParentFile();
		
		// secondary directory isn't used for WebLogic installation.
		File secondaryDirectory = new File("secondary-directory-isnt-used-for-WebLogic-installation.");				

		// create silent XML file
		Collection<String> lines = responseFileBuilder.getResponseForInstallation(oracleHomeDirectory, secondaryDirectory);
		...
		ExecutionResult result = assertionHelper.assertFileExist(responseFile, 
				messageProvider, 
				"cirfc.assert_responsefile_exists", 
				executionResult);
		...
		if (result.getState() == ExecutionState.SUCCESS ) {
			...
			// complete as success
	    	Object[] args = { responseFile };
	    	executionResult.completeAsSuccessful(messageProvider, "cirfc.succeed", args);
	    	return;							
		}
    	...
    	executionResult.completeAsFailure(messageProvider, "cirfc.failed", args2);		
    }    
