...
/**
 * <p>
 * Implementation of the <code>org.apache.commons.chain.Command</code> interface which invokes an URL sequences with
 * HTTP Get Method a specified number of times. The result is stored in a {@link HttpInvocationsSet} object.
 * </p>
 * 
 * <p>
 ...
 * 
 * <ul>
 * <li><code>urls</code> defines the URL sequence which should be accessed. The type is <code>java.lang.String[]</code>.
 * </li>
 * 
 * <li><code>requests</code> defines the number of times the URL sequence should be invoked. The type is
 * <code>int</code>.</li>
 * 
 * <li><code>reset</code> defines if HTTP session should reset after each sequence invocation. The type is
 * <code>boolean</code>.</li>
 * 
 * <li><code>execution-result</code> contains execution result object which is used 
 * to report the result of the execution of the command. The type is 
 ...
 * response properties which defines properties whose values is extracted from each
 * HTTP response and stored in each {@link HttpInvocationResult}. The type is 
 ...
 * <ul>
 * <li><code>results</code> contains an object containing the results of all the sequences. The type is
 ...
 * 
 * <p>
 * The command uses the <A HREF="http://hc.apache.org/httpclient-3.x/"> Apache HTTP CLient</A> as HTTP client. The
 * HTTPClient is configured to used a <code>SecureProtocolSocketFactory</code> implementation which accepts all SSL
 * certificates. The used implementation is {@link EasySSLProtocolSocketFactory} configured with the trust manager
 * {@link AcceptingSSLTrustManager} which accepts all certificate chains. The consequence is that certificate validation
 * is disabled for SSL and all certificates are accepted.
 * </p>
 * 
 */
public class InvokeHttpGetMethodCommand implements Command
{
    ...
    /**
     * Key used to identify property in context: Defines the URL sequence which should be accessed.
     */
    public static final String URLS_KEY = "urls";

    /**
     * Key used to identify property in context: Defines the number of times the URL which be invoked.
     */
    public static final String REQUESTS_KEY = "requests";

    /**
     * Key used to identify property in context: Defines if HTTP session should reset after each sequence invocation.
     */
    public static final String RESET_KEY = "reset";

    /**
     * Key used to identify property in context: Contains an HttpInvocationsSet containing the results of the invocations.
     */
    ...
    /**
     * Key used to identify property in context: Contains an HttpConfiguration containing the configuration
     * of the HTTP client.
     */
	...
	public boolean execute( Context context ) throws Exception
    {
        ...
        // create result set
        HttpInvocationsSet result = new HttpInvocationsSetImpl( urls, requests, reset, httpConfiguration.getProxyHost(), httpConfiguration.getProxyPort() );
	...
	void addExecutionInfo() {
        ...
        executionResult.addMessage(messageProvider.getMessage("ihgc.proxy_info")
        		, proxyAsString.toString() );      
        ...
        executionResult.addMessage(messageProvider.getMessage("ihgc.urls_info"), 
        		urlsAsString.toString() );        
	}
     ...
     * 
     * @return XML document if response could be parsed as XML, otherwise null is returned.
     */
     ...
     * 
	 * @param property Response property info object describing the property.
     * 
     ...
     * 
     * @param document DOM containing the response body.
	 * @param property Response property info object describing the property.
     * 
	...
	Object getPropertyFromBody(Document document, ResponsePropertyInfo propertyInfo ) {
        ...
        if (propertyValue != null) {
            ...
            {
                StringBuilder message = new StringBuilder();
                message.append( "Successfully completed property value extraction from HTTP response body, using XPath expression < " );
                message.append( propertyInfo.getXPath() );                
            ...
            if (propertyValue.equals("")) {
                ...
                {
                    StringBuilder message = new StringBuilder();
                    message.append( "Extracted property value from HTTP response body was empty string. Will return 'null'." );
                    logger.debug( message.toString() );
                }
            ...
            {
                StringBuilder message = new StringBuilder();
                message.append( "Extracted property value from HTTP response body was null, using XPath expression < " );
                message.append( propertyInfo.getXPath() );                
	...
	Object getPropertyFromHttpMethod(HttpMethod method, ResponsePropertyInfo propertyInfo ) {
		...
		try {
	        ...
	        {
	            StringBuilder message = new StringBuilder();
	            message.append( "Successfully completed property value extraction from HTTP method object, using XPath expression < " );
	            message.append( propertyInfo.getXPath() );                
	            message.append( "> which returned value <" );
	            message.append( ReflectionToStringBuilder.toString( propertyValue) );                
	            message.append( ">." );                            
	...
	Object getPropertyFromHttpState(HttpClient client, ResponsePropertyInfo propertyInfo ) {
		...
		try {
	        ...
	        {
	            StringBuilder message = new StringBuilder();
	            message.append( "Successfully completed property value extraction from HTTP state object, using XPath expression < " );
	            message.append( propertyInfo.getXPath() );                
	            message.append( "> which returned value <" );
	            message.append( ReflectionToStringBuilder.toString( propertyValue) );                
	            message.append( ">." );                            
