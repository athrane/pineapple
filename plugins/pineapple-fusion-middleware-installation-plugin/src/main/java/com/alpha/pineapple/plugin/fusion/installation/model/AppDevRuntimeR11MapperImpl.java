 ...
 * of the Application Development Runtime 11.x installer from the content of the model. 
 */
public class AppDevRuntimeR11MapperImpl implements ModelMapper {
	...
	public String getInstaller(FusionMiddlewareInstallation model) {
		return model.getApplicationDevelopmentRuntime().getRelease11().getInstaller();		
	}
	
	public String getUninstaller(FusionMiddlewareInstallation model) {
		return model.getApplicationDevelopmentRuntime().getRelease11().getUninstaller();
	}

	public File getTargetDirectory(FusionMiddlewareInstallation model) {
		return new File(model.getApplicationDevelopmentRuntime().getRelease11().getTargetDirectory());		
	}

	public File getLocalJvm(FusionMiddlewareInstallation model) {
		return new File(model.getApplicationDevelopmentRuntime().getRelease11().getLocalJvm());
	}	
