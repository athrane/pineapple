 ...
 * creates a silent XML file for JRockit R28 installation as described here:
...
public class CreateR28SilentXmlCommand implements Command {

	/**
	 * Silent XML file name. 
	 */
	static final String SILENTINSTALL_JROCKIT_XML = CreateR28SilentXmlCommand.class.getCanonicalName()+ ".xml";
    ...
    {    
        ...
        {
        	logger.debug( messageProvider.getMessage("cr28sxc.start" ) );        	
        }
        ...
        if ( logger.isDebugEnabled() ) 
        {
        	logger.debug( messageProvider.getMessage("cr28sxc.completed" ) );        	
        }
    ...
    {    	    	
		...
		if(silentXMLFile.exists()) {
			...
			if(!succeded) {
		    	...
		    	executionResult.completeAsFailure(messageProvider, "cr28sxc.delete_oldsilentxml_failed", args);
		    	return;				
			}			
		}
		...
		lines.add("     <data-value name=\"INSTALL_DEMOS_AND_SAMPLES\" value=\"false\" />");
		lines.add("     <data-value name=\"INSTALL_SOURCE_CODE\" value=\"false\" />");		
    	...
    	Object[] args = { silentXMLFile };    	        	
    	String description = messageProvider.getMessage("cr28sxc.assert_silentxml_exists", args );		
		...
		if (asserter.lastAssertionSucceeded()) {
	    	...
	    	executionResult.completeAsSuccessful(messageProvider, "cr28sxc.succeed", args);
	    	return;							
		}
		...
		// complete as failure
    	Object[] args2 = { silentXMLFile };
    	executionResult.completeAsFailure(messageProvider, "cr28sxc.failed", args);		
    }    
