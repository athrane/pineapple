...
public class ExecutionResultMapperImpl implements ExecutionResultMapper {
    ...
    /**
     * Map execution result notification to model result. Only the result itself
     * is mapped, children aren't mapped.
     * 
     * Messages are sorted by key before they are added.
     * 
     * Correlation ID is mapped from the object hash code. The correlation ID of
     * the parent result is also mapped from the parent result hash code.
     * 
     * @param notification
    ...
    /**
     * Returns true if model result is new. The result is interpreted as new if
     * it is doesn't exist in the result map, e.g. is registered under its
     * correlation ID.
     * 
     * @param modelResult
     *            model result.
     ...
     * 
     * If the result doesn't have a parent correlation ID defined then the
     * result is interpreted as the root result at the remote execution. The
     * parent correlation ID for the root result is resolved to index of the
     * local root result.
     * 
     * @param modelResult
