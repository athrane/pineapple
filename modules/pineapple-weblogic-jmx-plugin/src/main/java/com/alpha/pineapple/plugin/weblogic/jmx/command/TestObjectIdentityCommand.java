...
/**
 * <p>
 * Implementation of the <code>org.apache.commons.chain.Command</code> interface which tests the values
 * of two resolved attributes contained in a resolved type. The contained values are expected to objects. 
 * </p>
 * 
 * <p>
 ...
 * </p>        
 */
public class TestObjectIdentityCommand implements Command
{    	
    ...
    public boolean execute( Context context ) throws Exception
    {
        ...
        {
        	logger.debug( messageProvider.getMessage("toic.start") );        	
        }
        ...
        if ( logger.isDebugEnabled() ) 
        {
        	logger.debug( messageProvider.getMessage("toic.completed"));        	
        }
    ...
    void doTest( Context context ) throws Exception
    {
        ...
        if (!resolvedType.getPrimaryParticipant().isResolutionSuccesful() ) {
        	...
        	executionResult.completeAsFailure(messageProvider, "toic.primary_resolution_failed", args);
            return;
        }
        ...
        if (!resolvedType.getSecondaryParticiant().isResolutionSuccesful() ) {
        	...
        	executionResult.completeAsFailure(messageProvider, "toic.secondary_resolution_failed", args);
            return;
        }
        ...
        if ( value1 == null) {
            ...
            // primary and secondary is null
            if (value2 == null) {
        
                // set successful result            	
            	executionResult.completeAsSuccessful(messageProvider, "toic.null_succeed" );
                return;            	
            }
        	...
        	executionResult.completeAsFailure(messageProvider, "toic.not_equal_failed", args);
            return;
        }

        // primary isn't null but secondary is == not equal        
        if (value2 == null) {
            // set failed result
    		Object[] args = { resolvedType.getPrimaryParticipant().getValueAsSingleLineString(), 
    				resolvedType.getSecondaryParticiant().getValueAsSingleLineString() };    		
		...
		Object[] args = { resolvedType.getPrimaryParticipant().getValueAsSingleLineString(), 
				resolvedType.getSecondaryParticiant().getValueAsSingleLineString() };
    	executionResult.completeAsComputed(messageProvider, "toic.succeed", args);    	
    }
