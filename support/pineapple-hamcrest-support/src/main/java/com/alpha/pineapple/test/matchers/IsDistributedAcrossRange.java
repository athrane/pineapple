...
/**
 * Matches if the values in a collection are distributed across a set of legal
 * range of values.
 * 
 * The matcher succeeds if the values is precisely distributed among the values
 * in the range.
 */
public class IsDistributedAcrossRange<T> extends TypeSafeMatcher<Iterable<T>> {
	...
	public boolean matchesSafely(Iterable<T> items) {
		...
		for (T key : loadDistribution.keySet()) {
			...
			// test the load
			if (!isExpectedLoad(isMultiplum, expectedLoad, load))
				return false;
		}
	 ...
	 * 
	 * @param isMultiplum
	 *            Is true if number of entries is multiplum of the range.
	 * @param expectedLoad
	 *            The expected load for each entry in the range.
	 * @param load
	 *            The actual load for the current entry in the range.
	 * 
	 * @return true if the actual load is expected.
	 */
	boolean isExpectedLoad(boolean isMultiplum, double expectedLoad, int load) {
		if (isMultiplum) {
	...
	public void describeTo(Description description) {
		description.appendText("distributedAcrossRange(").appendValue(range).appendText(")");
	}
	 ...
	 * 
	 * @param items
	 *            Collection of entries which is distributed to node.
	 * @param map
	 *            Map of legal nodes.
	 * 
