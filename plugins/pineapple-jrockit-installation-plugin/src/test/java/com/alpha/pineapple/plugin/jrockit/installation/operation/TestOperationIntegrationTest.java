...
@ContextConfiguration( locations = { "/com.alpha.pineapple.testutils-config.xml", "/com.alpha.pineapple.plugin.jrockit.installation-config.xml" } )
public class TestOperationIntegrationTest {
	...
	Session session;
	 ...
	 /**
     * Mock message provider used by the session.
     */
	...
	/**
	 * Write mock R27 java.exe
	 * 
	 * @return File name to mock java.exe
	 * 
	 * @throws IOException If writing fails.
	 */
	File writeMockJavaR27Executable(String filename) throws IOException {

		//  create bin directory		
		File binDirectory = ObjectMotherIO.createSubDirectory(testDirectory, "bin");		
		
		// Initialize executable		
		File executableFile = new File(binDirectory, filename);
		
		// write installer
		Collection<String> lines = new ArrayList<String>();
		lines.add("@ECHO OFF");
		lines.add("@ECHO java version \"1.5.0_17\"");		
		lines.add("@ECHO Java(TM) 2 Runtime Environment, Standard Edition (build 1.5.0_17-b04)");
		lines.add("@ECHO BEA JRockit(R) (build R27.6.3-40_o-112056-1.5.0_17-20090318-2104-windows-ia32, compiled mode)");		
		
		// write the file
		FileUtils.writeLines(executableFile, lines);
	...
	public void testCanExecuteWithMinimalModel() throws Exception {
        ...
        // create content
        Object content = contentMother.createEmptyJRockitInstallation();

        // invoke operation
        testOperation.execute( content, session, executionResult );
	...
	/**
	 * Test that the operation can execute with a R27 model.
	 */
	@Test
	public void testCanExecuteWithR27Model() throws Exception {

		// write exe to pass file existence test 		
		writeMockJavaR27Executable("java.exe");
		
		// write runable cmd
		File installerFile = writeMockJavaR27Executable("java.cmd");		
		...
		// create content
        Object content = contentMother.createR27JRockitInstallation(installerFile.getAbsolutePath(), target);
	...
	/**
	 * Test that the operation can execute with a R28 model.
	 */
	@Test
	public void testCanExecuteWithR28Model() throws Exception {

		File installerFile = writeMockJavaR27Executable("java.exe");		
		String target = testDirectory.getAbsolutePath();
		...
		// create content
        Object content = contentMother.createR28JRockitInstallation(installerFile.getAbsolutePath(), target);
