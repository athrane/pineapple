 ...
 * which distributes a module to a remote Pineapple instance.
 * 
 ...
 * 
 * <ul>
 * <li><code>module</code> defines the name of the module which is distributed.
...
public class DistributeModuleCommand implements Command, ZipProgressCallback {
	...
	void doDistribute(Context context) {
		...
		try {
			...
			if (!compressModule(moduleDirectory, archiveFile))
				return;
			if (!uploadModule(archiveFile))
				return;
