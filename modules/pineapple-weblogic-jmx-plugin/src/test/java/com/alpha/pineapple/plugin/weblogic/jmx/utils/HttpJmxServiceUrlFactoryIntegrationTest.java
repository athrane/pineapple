...
/**
 * Integration test of the {@linkplain HttpJmxServiceUrlFactoryImpl} class. 
 */
@RunWith( SpringJUnit4ClassRunner.class )
@ContextConfiguration( locations = { "/com.alpha.pineapple.plugin.weblogic.jmx-config.xml" } )
public class HttpJmxServiceUrlFactoryIntegrationTest {

	/**
	 * Subject under test. 
	 */
	@Resource(name="httpJmxServiceUrlFactory")
	...
	/**
	 * Test service URL is created as expected for HTTP.
	 * 
	 * @throws Exception If test fails.
	 */
	@Test
	public void testCreateEditMBeanServerUrlForHttp() throws Exception {
		...
		JMXServiceURL url = factory.createEditMBeanServerUrl(JmxProtool.HTTP, randomHost, randomPort);				
		assertEquals(url.toString(), expected.toString());
	}

	/**
	 * Test service URL is created as expected for HTTPS.
	 * 
	 * @throws Exception If test fails.
	 */
	@Test
	public void testCreateEditMBeanServerUrlForHttps() throws Exception {
		StringBuilder expected = new StringBuilder()
			.append("service:jmx:")
			.append("https")
			.append("://")
		...
		JMXServiceURL url = factory.createEditMBeanServerUrl(JmxProtool.HTTPS, randomHost, randomPort);				
		assertEquals(url.toString(), expected.toString());
	}
	...
	public void testFailsCreateEditMBeanServerUrlForOtherProtocol() throws Exception {
		factory.createEditMBeanServerUrl(JmxProtool.RMI, randomHost, randomPort);
	}
