...
/**
 * Matches if a collection of cookie maps contains non equal values.
 * 
 * <p>
 * The matcher succeeds if: 1) All maps contains the same number of keys. 2) All
 * maps contains the same keys names. 3) All map contains different values in
 * the maps.
 * </p>
 */
public class IsCookieMapsContainingNonEqualValues extends TypeSafeMatcher<Iterable<Cookie[]>> {
	...
	protected boolean matchesSafely(Iterable<Cookie[]> cookiesCollection) {
		...
		// create matcher
		Matcher<CookieMapPair> matcher = InfrastructureMatchers.cookieMapsPairContainingNonEqualValues();
	...
	public void describeTo(Description description) {
		description.appendText("isCookieMapsContainingNonEqualValues");
	}
	...
	@Factory
	public static Matcher<Iterable<Cookie[]>> containsNonEqualValues() {
		return new IsCookieMapsContainingNonEqualValues();
	}
