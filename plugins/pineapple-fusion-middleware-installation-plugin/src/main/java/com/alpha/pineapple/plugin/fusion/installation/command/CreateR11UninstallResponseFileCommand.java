...
import static com.alpha.pineapple.plugin.fusion.installation.FusionMiddlewareInstallationConstants.RESPONSE_UNINSTALL_FILE;
 ...
 * creates a response file for uninstallation of a Fusion Middleware product.
 * 
...
public class CreateR11UninstallResponseFileCommand implements Command {
    ...
    /**
	 * Create response file for Application Development Runtime.
     * 
     * @param context
     *            Command context.
     * 
    ...
    {    	    	
		...
		// create file object		
		File responseFile = new File(tempDirectory, RESPONSE_UNINSTALL_FILE);
		...
		if(responseFile.exists()) {
			...
			if(!succeded) {
		    	...
		    	executionResult.completeAsFailure(messageProvider, "cr11urfc.delete_oldresponsefile_failed", args);
		    	return;				
			}			
		}

		// create silent XML file
		Collection<String> lines = responseFileBuilder.getResponseForUninstallation();
		...
		ExecutionResult result = assertionHelper.assertFileExist(responseFile, 
				messageProvider, 
				"cr11urfc.assert_responsefile_exists", 
				executionResult);
		...
		if (result.getState() == ExecutionState.SUCCESS ) {
			...
			// complete as success
	    	Object[] args = { responseFile };
	    	executionResult.completeAsSuccessful(messageProvider, "cr11urfc.succeed", args);
	    	return;							
		}
    	...
    	executionResult.completeAsFailure(messageProvider, "cr11urfc.failed", args2);		
    }    
