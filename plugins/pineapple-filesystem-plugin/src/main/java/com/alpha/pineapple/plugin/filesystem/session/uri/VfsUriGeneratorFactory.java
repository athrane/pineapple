...
public class VfsUriGeneratorFactory {
	 ...
	 * 
	 * @param protocol
	 *            Protocol to return a URI generator for.
	 * @param host
	 *            Host to generate URI's for.
	 * @param port
	 *            TCP used to access the host.
	 * @param user
	...
	public VfsUriGenerator createGenerator(String protocol, String host, String port, String user, String password) {
		if (protocol.equalsIgnoreCase("file"))
			return new FileUriGeneratorImpl();
		if (protocol.equalsIgnoreCase("ftp"))
			return new FtpUriGeneratorImpl(host, port, user, password);
		if (protocol.equalsIgnoreCase("ftps"))
			return new FtpsUriGeneratorImpl(host, port, user, password);
		if (protocol.equalsIgnoreCase("http"))
			return new HttpUriGeneratorImpl(host, port, user, password);
		if (protocol.equalsIgnoreCase("https"))
			return new HttpsUriGeneratorImpl(host, port, user, password);
		if (protocol.equalsIgnoreCase("smb"))
			return new SmbUriGeneratorImpl(host, port, user, password);
		if (protocol.equalsIgnoreCase("zip"))
			return new ZipUriGeneratorImpl();
		return null;
	}
}
