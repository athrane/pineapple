 ...
 * copies a local file to a SSH host.
 * 
 * <p>Precondition for execution of the command is definition of these keys in 
 * the context:
 * 
 * <ul>
 * <li><code>local-file</code> defines the path name to the local file to copy. The type is 
 * <code>java.lang.String</code>.</li>
 * 
 * <li><code>remote-file</code> defines the path name to the remote file. The type is 
 * <code>java.lang.String</code>.</li>
 *
 * <li><code>file-permissions</code> defines the file permissions for the remote file. The type is 
 * <code>java.lang.Integer</code>. The parameter is optional. If the parameter is -1 then
 * file permissions isn't set on the remote file.</li>
 *
 * <li><code>user-ownership</code> defines the user ownership of the remote file. The type is 
 * <code>java.lang.Integer</code>. The parameter is optional. If the parameter is -1 then
 * ownership isn't set on the remote file.</li>
 * 
 * <li><code>group-ownership</code> defines the group ownership of the remote file. The type is 
 * <code>java.lang.Integer</code>. The parameter is optional. If the parameter is -1 then
 * ownership isn't set on the remote file.</li>
 * 
 * <li><code>substitute-variables</code> defines whether the content of the source file 
 * will processed for variable substitution. The type is <code>java.lang.Boolean</code>.</li>
 * 
 * <li><code>session</code> defines the agent session used communicate with an agent. 
 * The type is <code>com.alpha.pineapple.plugin.ssh.session.SshSession</code>.</li>
 * 
...
public class SecureCopyToCommand implements Command {
     ...
     * 
     * If a null file permissions value is supplied to the command then no permissions is set.
     * 
     * @param sftpChannel FTP channel.
     * 
     * @throws SftpException if operation fails.
     */
	void setFilePermissions(ChannelSftp sftpChannel) throws SftpException {
		if(skipSettingFilePermissions()) return;
     ...
     * Set user ownership. 
     * 
     * If a null user ownership value is supplied to the command then no permissions is set.
     * 
     * @param sftpChannel FTP channel.
     * 
     * @throws SftpException if operation fails.
	...
	void setUserOwnership(ChannelSftp sftpChannel) throws SftpException {
		if(skipSettingUserOwnership()) return;
	...
	void setGroupOwnership(ChannelSftp sftpChannel) throws SftpException {
		if(skipSettingGroupOwnership()) return;
     ...
     * 
     * @param source source file whose content should be processed.
     * 
     * @return if variable substitution is enabled then the path to the processed file is returned. Otherwise
     * the path to the original source file is returned.
     * 
     * @throws VariableSubstitutionException if processing fails.
     */
	File doVariableSubstitution(File source) throws VariableSubstitutionException {
		//return source;
		if(!this.substituteVariables) return source;
